...
void insert_val(char *name, char *v, int line,int scope) // inserts values into the table when initialised
{
	if (t->head == NULL)
		return;
	symbol *curr = t->head;
	while (curr != NULL)
	{
		if (strcmp(curr->name, name) == 0 && scope==curr->scope)
		{
			strcpy(curr->val, v);
			curr->line = line;
			return;
...
int retrieve_scope(char *name) // retrieves type from symbol table
{
	int scope = 1;
	if (t->head == NULL)
		return scope;
